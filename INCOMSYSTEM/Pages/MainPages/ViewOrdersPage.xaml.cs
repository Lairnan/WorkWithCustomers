...
namespace INCOMSYSTEM.Pages.MainPages
{
    public partial class ViewOrdersPage : Page
    {
        ...
        private void FormAgreementOrderMenu_Click(object sender, RoutedEventArgs e)
        {
            ...
            if (MessageBox.Show("Договор успешно сохранён. Открыть?", "Успешное сохранение", MessageBoxButton.YesNo, MessageBoxImage.Question) 
        ...
        private static bool FormAgreement(string path, Orders order)
        {
            ...
            while (true)
            {
                try
                {
                    ...
                    var employeePatronymic = order.Employees.patronymic != null ? $" {order.Employees.patronymic}" : string.Empty;
                    doc.ReplaceWordText("{executorName}", $"{order.Employees.surname} {order.Employees.name}{employeePatronymic}");
                ...
                {
                    var result = MessageBox.Show("Для сохранения записи, необходимо закрыть Word документ", "Ошибка", MessageBoxButton.YesNo, MessageBoxImage.Error);
